...
const profile = {
    ...
    netflix:{
        ...
        saveTime: false,
        ignore: '.touchable, .PlayerControls--control-element',
...
function kbControl(_controllerOptions){
    ...
    this.timer = {};
    this.amend = (action, check, intendedState, log = console.log('[%o, %o] %o === %o = %o', check, intendedState, check[0][check[1]], intendedState, check[0][check[1]] === intendedState)) => check[0][check[1]] === intendedState ? this.options.action[action](intendedState) : void 0;
    this.verify = (action, check, intendedState) => (this.timer[action] = window.setTimeout(this.amend, 10000/60, action, check, intendedState));
    this.setPseudo = (video, control) =>{
        ...
        this.options.action.playpause = (forced = video.paused) => this.verify('playpause', [video, 'paused'], video.paused ? !!this.options.action.play() : !!this.options.action.pause());
    };
    this.handlers = {
